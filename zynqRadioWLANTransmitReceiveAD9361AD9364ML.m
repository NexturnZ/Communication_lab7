...
% FMCOMMS2/3/4'| with |'ZedBoard and FMCOMMS2/3/4'| or |'ADI RF SOM'| in
% the |deviceNameSDR| variable to run with ZedBoard(TM) and ADI
% FMCOMMS2, FMCOMMS3, FMCOMMS4 hardware or ADI RF SOM.
...
% until the release method of the sdrTransmitter object is called. Messages are
...
% *Receiver Setup*
% 
% The receiver is controlled using the properties defined in the |sdrReceiver|
...
sdrReceiver.NumFramesInBurst = 1; % Capture all packets in a single burst 
...
burstCaptures = zeros(sdrReceiver.SamplesPerFrame,1);
...
while len == 0
    ...
    % samples, burstCaptures holds sdrReceiver.PacketPerBurst number of
    % consecutive packet worth of baseband WLAN samples
    [burstCaptures,len,lostSamples] = sdrReceiver();
end
if lostSamples
